 ...
 *
 * Source: birabittoh/rexglue-sdk tag sotn-nightly-20260817-7766f971,
 * ported onto the v0.9.0 baseline. Local changes: the extraction target
 * is game/, and ProcessTitleUpdate accepts loader-applied raw sibling
 * default.xexp patches.
 */
...
enum class SiblingTitleUpdateStatus {
  kSuccess,
  kHashMismatch,
  kExtractionFailed,
};

struct SiblingTitleUpdateResult {
  SiblingTitleUpdateStatus status = SiblingTitleUpdateStatus::kExtractionFailed;
  uint32_t files = 0;
};

/// Verify the embedded patch before merging any other package files into the game root.
SiblingTitleUpdateResult ExtractSiblingTitleUpdateTo(const std::filesystem::path& tu_path,
                                                     const std::filesystem::path& out_dir,
                                                     std::string_view expected_xexp_sha256) {
  FileReader reader(tu_path);
  if (!reader.ok()) {
    REXLOG_ERROR("Failed to open TU: {}", tu_path.string());
    return {};
  ...
  auto entries = ParseStfsFileTable(reader, *info);
  if (entries.empty()) {
    REXLOG_ERROR("No STFS entries found in TU: {}", tu_path.string());
    return {};
  }
  ...
  std::filesystem::create_directories(out_dir);
  auto xexp_path = out_dir / "default.xexp";
  {
    std::ofstream ofs(xexp_path, std::ios::binary);
    if (!ofs) {
      REXLOG_ERROR("Cannot write {}", xexp_path.string());
  ...
  if (!HexEqual(actual, expected_xexp_sha256)) {
    ...
    if (ec) {
      REXLOG_WARN("Could not remove rejected {}: {}", xexp_path.string(), ec.message());
    }
    return {SiblingTitleUpdateStatus::kHashMismatch, 0};
  }

  auto result = ExtractStfsTree(reader, entries, *info, out_dir, &entries[*xexp_index]);
  if (!result.complete) {
    REXLOG_ERROR("Sibling TU extraction incomplete ({} data files written before failure)",
                 result.files);
    return {};
  }
  return {SiblingTitleUpdateStatus::kSuccess, result.files + 1};
}
...
static bool ProcessTitleUpdate(const std::filesystem::path& dir,
  ...
  // The loader applies raw sibling patches without an update: device or STFS
  // package.
  if (!settings.sibling_xexp_sha256.empty()) {
    ...
    std::string msg =
        "This build of the game requires its title update patch "
        "(default.xexp) next to default.xex.\n\n"
        "Select the original title update package or its extracted "
        "default.xexp file.";
    while (true) {
      ...
      std::vector<SDL_DialogFileFilter> tu_filters = {
          {"Title update package or extracted patch", "*"}};
      std::string selected;
      if (!RunFileDialog(selected, tu_filters)) {
        continue;
      }

      if (IsStfsPackage(selected)) {
        auto result = ExtractSiblingTitleUpdateTo(selected, dir, settings.sibling_xexp_sha256);
        if (result.status == SiblingTitleUpdateStatus::kHashMismatch) {
          ShowErrorBox("SHA-256 Mismatch",
                       "The selected file does not match the expected title "
                       "update.\n\n"
                       "Please select the correct package or default.xexp.");
          continue;
        }
        if (result.status == SiblingTitleUpdateStatus::kExtractionFailed) {
          ShowErrorBox("Extraction Failed",
                       "Failed to extract the title update.\n\n"
                       "The file may be damaged or not a valid title update package.");
          continue;
        }
        REXLOG_INFO("Sibling title update extracted: {} files", result.files);
        return true;
      }
      ...
      std::error_code ec;
      std::filesystem::copy_file(selected, xexp_path,
                                 std::filesystem::copy_options::overwrite_existing, ec);
      if (ec) {
        REXLOG_ERROR("Failed to copy {} to {}: {}", selected, xexp_path.string(), ec.message());
        ShowErrorBox("Copy Failed",
                     "Could not copy the title update next to default.xex:\n\n" + ec.message());
        continue;
      }
      REXLOG_INFO("Title update patch copied to {}", xexp_path.string());

      // Title updates can carry data files the disc lacks; merge a Resource
      // tree found beside the selected patch into the game root.
      auto resource_src = std::filesystem::path(selected).parent_path() / "Resource";
      std::error_code merge_ec;
      if (std::filesystem::is_directory(resource_src, merge_ec) &&
          !std::filesystem::equivalent(resource_src, dir / "Resource", merge_ec)) {
        std::filesystem::copy(resource_src, dir / "Resource",
                              std::filesystem::copy_options::recursive |
                                  std::filesystem::copy_options::overwrite_existing,
                              merge_ec);
        if (merge_ec) {
          REXLOG_WARN("Could not merge update data from {}: {}", resource_src.string(),
                      merge_ec.message());
        } else {
          REXLOG_INFO("Merged title update data files from {}", resource_src.string());
        }
      }
      return true;
    }
...
bool EnsureGameDataImpl(const GameDataSelectorSettings& settings) {
  ...
  std::filesystem::path out_dir = exe_dir / "game";
